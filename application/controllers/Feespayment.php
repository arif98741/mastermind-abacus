...
class Feespayment extends Admin_Controller
{
            ...
            if ($response->isSuccessful()) {
                ...
                if (isset($paypalResponse['PAYMENTINFO_0_ACK']) && $paypalResponse['PAYMENTINFO_0_ACK'] === 'Success') {
                    if ($purchaseId) {
                        ...
                        set_alert('success', translate('payment_successful'));
        ...
        if (!empty($params)) {
            ...
            $array = [
                'paymentData' => [
                    ...
                    'ref_no' => $params['invoice_no'],
                    'order_code' => $params['invoice_no'],
                    'success_url' => base_url('/userrole/invoice?payment=success&ref=bkash'),
                    'failed_url' => base_url('/userrole/invoice?payment=failed&ref=bkash')
                ]
            ];

            $this->load->view('gateway/bkash/view', $array);
        }
    }
     ...
     */
    public function executeBkashPayment()
    {
        $this->load->library('bkash_payment');
        $paymentId = $_POST['paymentId'];
        $execution = $this->bkash_payment->executePayment($paymentId);
        $responseArray = json_decode($execution, true);

        if (array_key_exists('transactionStatus', $responseArray)
            && $responseArray['transactionStatus'] == 'Completed') {

            $params = $this->session->userdata('params');
            $ref_id = $responseArray['trxID'];
            $arrayFees = array(
                'allocation_id' => $params['allocation_id'],
                'type_id' => $params['type_id'],
                'collect_by' => "",
