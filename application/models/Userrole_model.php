...
//        exit;
        ...
        if ($this->session->userdata['loggedin_role_id'] == 7) {
            ...
            //////////////////find student class and section
            $this->db->select('*');
            $this->db->from('enroll');
            $this->db->where('student_id', $this->session->userdata['loggedin_userid']);
            $data = $this->db->get();
            $studentEnroll = (array)$data->result();
            $class = array_column($studentEnroll, 'class_id');
            $section = array_column($studentEnroll, 'section_id');
            //////////////////find student class and section
            ...
            $this->db->from('staff');
            $this->db->join('login_credential', 'login_credential.user_id = staff.id and and login_credential.role = "7"', 'inner');
            $this->db->join('roles', 'roles.id = login_credential.role', 'left');
            $this->db->join('staff_designation', 'staff_designation.id = staff.designation', 'left');
            $this->db->join('staff_department', 'staff_department.id = staff.department', 'left');
            $this->db->join('teacher_allocation', 'staff.id = teacher_allocation.teacher_id');
            $this->db->join('enroll', 'enroll.class_id = teacher_allocation.class_id');
            if ($branchID != "") {
                $this->db->where('staff.branch_id', $branchID);
            }

            $this->db->where('enroll.student_id', $this->session->userdata['loggedin_userid']);
            $this->db->where_in('teacher_allocation.class_id', $class);
            $this->db->where_in('teacher_allocation.section_id', $section);
